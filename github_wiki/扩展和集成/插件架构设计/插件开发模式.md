...
- 工厂模式
  ...
  - 参考路径
    - [工厂方法 newAction](utils.md)
    - [工厂方法 newIcon](utils.md)
    - [工厂方法 newButton](utils.md)
    - [主窗口动作装配示例](PPOCRLabel.md)
...
- 观察者模式（信号槽）
  ...
  - 参考路径
    - [Canvas 信号定义](canvas.md)
    - [主窗口连接信号示例](PPOCRLabel.md)
...
- 策略模式（OCR 算法选择）
  ...
  - 参考路径
    - [OCR 初始化与预测](PPOCRLabel.md)
    - [语言切换重建 OCR](PPOCRLabel.md)
...
- 图像预处理与模型推理
  ...
  - 参考路径：[OCR 初始化与设备选择](PPOCRLabel.md)
- 线程与 UI 刷新
  - 自动识别使用独立线程，避免阻塞主线程；仅在必要时通过信号传递少量数据，减少主线程压力。
  - 参考路径：[Worker.run 与信号发射](autoDialog.md)
- 画布渲染
  - Canvas 在 paintEvent 中按需绘制，避免不必要的重绘；合理设置字体大小与高亮绘制。
  - 参考路径：[Canvas.paintEvent](canvas.md)
- 列表与编辑
  - 唯一标签列表与就地编辑列表在频繁更新时注意批量操作与滚动同步，避免闪烁。
  - 参考路径：[EditInList 就地编辑](editinlist.md)
...
- OCR 初始化失败
  ...
  - 参考路径：[OCR 初始化与预测](PPOCRLabel.md)
- 自动识别无响应或卡死
  - 现象：进度条不动或对话框无法关闭。
  - 排查：查看 Worker 线程日志；确认图像尺寸与格式；必要时中断线程。
  - 参考路径：[AutoDialog 与 Worker](autoDialog.md)
- 画布交互异常
  - 现象：无法拖拽、选择或缩放。
  - 排查：检查 Canvas 的信号是否正确连接；确认鼠标事件与键盘事件处理。
  - 参考路径：[Canvas 鼠标/键盘事件](canvas.md)
- 设置加载失败
  - 现象：设置未生效或丢失。
  - 排查：确认设置文件路径与权限；尝试重置设置。
  - 参考路径：[Settings.load/save/reset](settings.md)
- 国际化字符串缺失
  - 现象：界面文字未翻译或报错。
  - 排查：检查资源文件路径与编码；确认回退链路。
  - 参考路径：[StringBundle 回退与加载](stringBundle.md)
...
- 新建模块
  ...
  - 参考路径：[工厂方法 newAction/newIcon/newButton](utils.md)
- 注入 UI
  - 在主窗口中创建工具按钮或停靠面板，连接到插件模块。
  - 参考路径：[主窗口动作装配示例](PPOCRLabel.md)
- 事件集成
  - 通过 Canvas 的 selectionChanged/shapeMoved/newShape 等信号接入插件逻辑。
  - 参考路径：[Canvas 信号定义](canvas.md)
- 数据与状态
  - 使用 Settings 进行必要的持久化；使用 StringBundle 支持多语言。
  - 参考路径：[Settings](settings.md)、[StringBundle](stringBundle.md)
- 线程与性能
