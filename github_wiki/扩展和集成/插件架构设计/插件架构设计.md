...
- 添加工具栏按钮
  ...
  - 参考路径：[libs/toolBar.py](toolBar.md)
- 添加菜单项与快捷键
  - 使用动作构造函数统一创建 QAction，支持图标、提示、快捷键与触发回调。
  - 参考路径：[libs/utils.py](utils.md)
  - 示例动作创建与快捷键绑定：[PPOCRLabel.py](PPOCRLabel.md)
- 连接信号槽
  - Canvas 的 zoomRequest、scrollRequest、newShape、selectionChanged、shapeMoved 等信号用于驱动 UI 更新与状态同步。
  - 参考路径：[libs/canvas.py](canvas.md)
  - 典型连接示例：[PPOCRLabel.py](PPOCRLabel.md)
- 启动自动识别流程
  - 通过 AutoDialog 与 Worker 线程实现后台推理与进度反馈。
  - 参考路径：[libs/autoDialog.py](autoDialog.md)
...
- 当前集成
  ...
  - 参考路径：[PPOCRLabel.py](PPOCRLabel.md)
- 可插拔接口建议
...
- 工厂模式
  ...
  - 参考路径：[libs/utils.py](utils.md)、[libs/stringBundle.py](stringBundle.md)
- 观察者模式
  - Canvas 通过信号（pyqtSignal）向 MainWindow 发布状态变化，实现跨组件的松耦合通知。
  - 参考路径：[libs/canvas.py](canvas.md)
- 策略模式
  - 将 OCR 推理封装为可替换策略，配合配置键与工厂方法实现动态切换。
  - 参考路径：[PPOCRLabel.py](PPOCRLabel.md)、[libs/autoDialog.py](autoDialog.md)
...
- 配置管理
  ...
  - 参考路径：[libs/settings.py](settings.md)、[libs/constants.py](constants.md)
- 错误处理
...
- 颜色/字体显示异常
  ...
  - 参考路径：[libs/colorDialog.py](colorDialog.md)、[libs/canvas.py](canvas.md)
- 配置丢失或加载失败
  - 查看设置文件路径与权限；必要时重置设置。
  - 参考路径：[libs/settings.py](settings.md)
...
## 附录
- 新增工具栏按钮步骤
  - 使用工具栏基类与按钮尺寸规范：[libs/toolBar.py](toolBar.md)
  - 创建动作并绑定回调：[libs/utils.py](utils.md)、[PPOCRLabel.py](PPOCRLabel.md)
- 新增菜单项与快捷键步骤
  - 通过 newAction 构造动作，设置图标、提示与快捷键：[libs/utils.py](utils.md)
  - 将动作加入菜单/工具栏：[PPOCRLabel.py](PPOCRLabel.md)
- 新增 OCR 模型接入步骤
  - 定义统一推理接口与配置键：[libs/constants.py](constants.md)
  - 在 MainWindow 初始化并注入策略：[PPOCRLabel.py](PPOCRLabel.md)
  - 后台线程执行推理与进度反馈：[libs/autoDialog.py](autoDialog.md)
